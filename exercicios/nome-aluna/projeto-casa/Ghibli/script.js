...
async function getData() {
    try {
        ...
        // json.slice(5,10).forEach(element => { - pra selecionar um trecho
        ...
        json.forEach(element => {
            ...
            const item_filme = filmeElement.querySelectorAll('span')
            const item_fotos = filmeElement.querySelector('#foto')
            const movie_banner = filmeElement.querySelector('#poster')
            item_filme[0].innerText = element.title
            item_filme[1].innerText = element.original_title
            item_filme[2].innerText = element.original_title_romanised
            item_fotos.setAttribute('src', element.image);
            movie_banner.setAttribute('src', element.movie_banner);
            item_filme[5].innerText = element.description
            item_filme[6].innerText = element.director
            item_filme[7].innerText = element.producer
            item_filme[8].innerText = element.release_date
        ...
        console.log ("Eita deu ruim: " + erro)
    }
}
